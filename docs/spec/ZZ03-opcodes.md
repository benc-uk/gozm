...
- **Binary**: Binary opcode value (8 bits)
...
| 0OP   | 14  | 190  | be  | 10111110 | E    |      |     |     |     | Extended opcode (see EXT)  |
...
| EXT   | —   | 14   | 0e  | 00001110 | E    | ?    |      |     |     | (unused)                                        |
| EXT   | —   | 15   | 0f  | 00001111 | E    | ?    |      |     |     | (unused)                                        |
...
The key insight from this comprehensive analysis is that **a complete opcode identification requires both the operand count category (0OP, 1OP, 2OP, VAR, EXT) AND the opcode number**, as the same opcode number can exist across different categories with entirely different meanings. This table makes all variants explicit, including different argument type encodings for the same logical instruction.
